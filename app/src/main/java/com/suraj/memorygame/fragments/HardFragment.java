...
public class HardFragment extends Fragment {
    ...
    TextView timer_hard, moves_hard;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        recyclerView = view.findViewById(R.id.recycler_hard);
        timer_hard = view.findViewById(R.id.text_timer_hard);
        moves_hard = view.findViewById(R.id.text_hard_moves);

        cards = new ArrayList<>();

        //set a GridLayoutManager with 3 number of columns
        GridLayoutManager gridLayoutManager = new GridLayoutManager(requireActivity(),4, LinearLayoutManager.VERTICAL,false);
        recyclerView.setLayoutManager(gridLayoutManager);

        shuffleCards(CARDS,Global.HARD_NO_OF_CARDS);
        shuffleCards(CARDS,Global.HARD_NO_OF_CARDS);
        ...
        new CountDownTimer(Global.HARD_TIME,Global.TIMER_INTERVAL){
            @Override
            public void onTick(long millisUntilFinished) {
                if (isPaused || isCancelled){
                    ...
                    timer_hard.setText((millisUntilFinished / Global.TIMER_INTERVAL+" sec left"));
                    RemainingTime = millisUntilFinished;
                    if (count == Global.HARD_NO_OF_CARDS) {
                        SharedPreferences.Editor editor = sharedPreferences.edit();
                        editor.putString("time", String.valueOf(((Global.HARD_TIME - millisUntilFinished)/ Global.TIMER_INTERVAL)));
                        editor.putString("move", String.valueOf(move));
                        editor.apply();
                    ...
                    if (cards.get(pos).equals(cards.get(position))){
                        ((EasyFlipView) v).setOnFlipListener((easyFlipView, newCurrentSide) -> {
                            ...
                            new Handler().postDelayed(() -> {
                                move++;
                                moves_hard.setText(Integer.toString(move));
